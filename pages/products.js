...
export default function Products() {
	...
	const handleSearchResult = () => {
		...
		const filteredProduct = allProducts.filter(p =>
			p.title.toLowerCase().includes(searchKey.toLowerCase().trim())
		);
	...
	const renderCategoryList = () => {
		...
		return (
			<div className="flex justify-around mt-5">
				{[...new Set(state.product.map(c => c.category))].map((l, i) => (
					<p key={i} onClick={handleCategoryClick()} className="bg-[#8AAAE5] p-1 rounded-md">
						{l}
					</p>
				))}
			</div>
